...
- **黑曜石玻璃美学**：全新 Celestial Obsidian Glass 设计语言，12pt 连续曲率卡片与呼吸状态信标，兼具深邃质感与专业易用性。

---

## ✨ 核心特性

- **🌌 沉浸式 Celestial Obsidian 原生界面**：
  - **Hero Command Deck**：主控概览仪表盘，动态呼吸信标（StatusBeaconDot）实时映射核心运行状态；
  - **Elevated Glass Card**：标准化 12pt 连续曲率（Continuous Corner Radius）与 Morandi 上下行网络示波；
  - **原生状态栏与流式交互**：基于现代 `NSPopover` 架构，测速时不关闭菜单，节点延迟逐个流式点亮；
...
- **🔍 独立抓包分析视窗与规则推演 (`⌘D`)**：
  - 支持按客户端或域名筛选的高密度 10 列请求流水表与单连接详细抽屉（Detail Drawer）；
  - 内置规则仿真求值器（Rule Evaluator），实时推演匹配规则与最终出站策略；
  - 单击/右击任意连接，一键将目标域名或 IP 沉淀为直连、代理或拦截规则。
- **🌐 全协议栈解析与深度适配**：
  - 严格 sing-box 1.14+ 规范对齐，原生支持 WireGuard、Hysteria 2（混淆/带宽）、TUIC v5、ShadowTLS、VLESS、Trojan、Shadowsocks 等主流协议；
  - 统一 `Default` 配置集与多订阅聚合，单源/多源自动清洗无效字段与广告节点。
- **🛡️ 工业级特权边界**：
...
本项目遵循严苛的架构治理准则（单一事实来源、零废弃冗余逻辑、原子事务化变更与严格防死锁机制），代码维护与新增特性须遵循 [AGENT.md](AGENT.md) 规则守则。
